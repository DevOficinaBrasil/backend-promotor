...
| Recipient on a queued row | Not stored; shown in `status` by join | User's explicit call 2026-08-14. Storing it at enqueue costs a query per batch and writes a value the dispatch overwrites anyway, so the column would read as fact while being a guess. The join answers the real question — who would be chosen *now* — and picks up a phone corrected overnight | y |
...
3. WHEN the operator runs the console's `status` command THEN the system SHALL print counts by STATUS, the number of rows due now, and the next rows to go out with their `AVAILABLE_AT`, `ATTEMPTS`, `LOCKED_BY`, `ERRO_ENVIO` and the recipient who would be chosen if the send happened now — resolved by join at read time, never stored on the queued row.
