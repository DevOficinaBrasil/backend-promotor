...
| `xlsx` (SheetJS) | dependência já instalada, hoje sem nenhum uso no código (`package.json`) | Primeiro uso real. Para `.xlsx` (ZIP), `XLSX.read(buffer, { type: "buffer" })`. Para `.csv`, o buffer é decodificado para string **antes** (UTF-8, com fallback para latin1 se inválido) e entregue via `type: "string"` — o parser CSV do SheetJS não assume UTF-8 por padrão e corrompia acentuação em CSVs genuinamente UTF-8 (bug real, corrigido pós-entrega; ver nota em `tasks.md`). |
...
**Correção pós-Verifier (fix-round 1)**: o Verifier independente apontou que o risco real do `UNION ALL` não era lat/long (esse já se prova seguro — o ramo existente já faz `radians(ce."latitude")` sem cast, então a coluna já é numérica no DW), e sim as outras colunas de texto (`numero`, `cep`, `telefone`, etc.) — seus tipos reais no DW só são conhecidos por `entities/CadastroEmpresa.ts`, arquivo já comprovadamente impreciso para tipos (erra `latitude`/`longitude` e `id_oficina`, ver `utils/sqlCadastroEmpresa.ts:81`). Um mismatch ali derrubaria as 3 queries para todo cliente, não só quem tem oficina importada. Corrigido castando `::text` todas as colunas de texto dos dois ramos em `getComunityNearbyOficinas` e `getCommunityOficinas` (`countCommunityOficinas` só seleciona `"ID_OFICINA"`, sem risco). Lat/long seguem `::double precision` (já confirmado seguro).

**Verificação pendente contra banco real**: mesmo com os casts, a query completa nunca foi executada contra Postgres real nesta sessão — instrução explícita de nunca acessar o banco durante Specify/Design/Tasks/Execute. Validar manualmente antes do deploy (sintaxe, planos de execução, índices).
...
| Atribuição de rota | **[Atualizado pós-entrega, ver Fix de Performance em `tasks.md`]** `RotaService.atribuirComContexto` + `prepararContextoAtribuicaoLote`, calculado uma vez por importação — não mais uma chamada de `assignOficinaFromCommunitySignup` por oficina (decisão original, abaixo) | Mesmo algoritmo (raio, Haversine, idempotência), mas sem repetir por linha as consultas que valem para o arquivo inteiro. `assignOficinaFromCommunitySignup` continua intocado e é quem `atribuirRota` (ainda existe, não é mais chamado pelo loop principal) usa |
| ~~Atribuição de rota (decisão original, substituída)~~ | ~~Reaproveitar `RotaService.assignOficinaFromCommunitySignup` tal como está, chamando uma vez por oficina vinculada~~ | ~~Já implementa exatamente o algoritmo necessário — evita duplicar lógica de atribuição só para este fluxo~~ — motivo da mudança: essa chamada por linha refazia consultas idênticas para o arquivo inteiro (`getActiveCampanhasBySlug`, `getCandidatosPorCampanhas`), dominando o tempo de importação de arquivos grandes |
| Escopo da atribuição de rota | Todas as campanhas **ativas** do `EMPRESA_SLUG` (não só a `ID_CAMPANHA` do import) | Preservado na reescrita de performance: `prepararContextoAtribuicaoLote` também carrega todas as campanhas ativas do slug, não só a do import — e é o comportamento mais correto: uma oficina importada deveria poder ser coberta por qualquer campanha ativa do cliente, não só a que o admin tinha aberta no wizard |
